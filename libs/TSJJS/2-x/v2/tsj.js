/*!
   ...
   *31th August 2019 ( 5th Update )
   *by Debarchito Nath ( MOD ER HACKS ) © Copyright | 2019 | ALL RIGHTS RESERVED
   *Includes more simpler, super lighter and powerfull  "tsjStyler JS" v1.4
   *Syntax optimised to support more browsers ( Supported by IE9+, Chrome, Firefox, Safari, Edge, Opera etc. ) and uses the extensible "Fibre DOM" which is the most flexible DOM ever
...
var randomVar_66j = GET(sel); for(let key in attributes) { randomVar_66j.setAttribute(key, attributes[key]); } return randomVar_66j; document.body.appendChild(this.parentElement) || document.documentElement.appendChild(this.parentElement); }; 

tsj.docWrite = function(val) { return document.write(val); }; tsj.write = tsj.append = function(sel, txt) { var randomVar_008eff = GETAll(sel); randomVar_008eff.innerHTML += txt; };  tsj.fWrite = tsj.formatWrite = function(sel, txt) { var randomVar_008eff = GETAll(sel); randomVar_008eff.innerHTML = txt; }; tsj.prepend = function(sel, txt) { var randomVar_018fff = GETAll(sel); randomVar_018fff.innerHTML = txt + randomVar_018fff.innerHTML; }; tsj.cWrite = tsj.createWrite = tsj.createElementWrite = function(sel, crt, attributes, cnt) { var randomVar_Huu56j = GET(sel);
...
document.documentElement.appendChild(this.parentElement); }; tsj.br = function() { return tsj.cElement("br"); }; tsj.hr = function() { return tsj.cElement("hr"); };

tsj.click = tsj.tap = function(sel, clk) { sel = sel ? sel : this; var randomVar_216963 = GETAll(sel); randomVar_216963.onclick = clk; }; tsj.dblclick = tsj.dbltap = function(sel, clk) {
...
tsj.cSelect = tsj.clickSelect = function(obj) { var text_val=eval(obj); text_val.focus(); text_val.select(); r = text_val.createTextRange(); if (!r.execCommand) return; r.execCommand('copy'); }; tsj.copyContent = tsj.copyCnt = tsj.cContent = tsj.cCnt = function(sel) {
...
tsj.fadeIn = function(sel, tm, sty) { tm = tm ? tm : 950; sty = sty ? sty : "ease"; tsj.css(sel + "{ -webkit-animation: DomfadeIn " + tm + "ms " + sty + "; animation: DomfadeIn " + tm + "ms " + sty + "; -webkit-animation-fill-mode: both; animation-fill-mode: both; } @-webkit-keyframes DomfadeIn { 0% {opacity: 0 !important;} 100% {opacity: 1!important; display : none; } } @keyframes DomfadeIn { 0% {opacity: 0; } 100% {opacity: 1!important; display : none;} } .DOM-1-CSS-fadeIn { -webkit-animation-name: DomfadeIn;  animation-name: DomfadeIn; } "); tsj.aClass(sel, "DOM-1-CSS-fadeIn"); }; tsj.fadeOut = function(sel, tm, sty) { tm = tm ? tm : 950; sty = sty ? sty : "ease"; tsj.css(sel + "{ -webkit-animation: DomfadeOut " + tm + "ms " + sty + "; animation: DomfadeOut " + tm + "ms " + sty + "; -webkit-animation-fill-mode: both; animation-fill-mode: both; } @-webkit-keyframes DomfadeOut { 0% {opacity: 1 !important;} 100% {opacity: 0!important; display : none; } } @keyframes DomfadeOut { 0% {opacity: 1 !important;} 100% {opacity: 0; display : none;} } .DOM-1-CSS-fadeOut { -webkit-animation-name: DomfadeOut;  animation-name: DomfadeOut; } "); tsj.aClass(sel, "DOM-1-CSS-fadeOut"); }; tsj.fadeInOut = function(sel, tm, sty) { tm = tm ? tm : 950; sty = sty ? sty : "ease-in-out"; tsj.css( sel + "{ opacity :0; animation:fadingh " + tm + "ms " + sty +" ; -webkit-animation:fadingh " + tm + "ms " + sty +" ; } @keyframes fadingh { 0%{opacity:0}50%{opacity:1}100%{opacity:0} } "); }; tsj.fadeOutIn = function(sel, tm, sty) { tm = tm ? tm : 950; sty = sty ? sty : "ease-in-out"; tsj.css( sel + "{animation:fadingg " + tm + "ms " + sty +" ; -webkit-animation:fadingg " + tm + "ms " + sty +" ; }@keyframes fadingg{0%{opacity:1}50%{opacity:0}100%{opacity:1}}"); }; 
