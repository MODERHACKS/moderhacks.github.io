/*!
   ...
   *13th September 2019 ( 8th Update )
...
var GET = function(sel) { return document.querySelector(sel); }; var GETAll = function(selector) { var rV_23_nodeList = document.querySelectorAll(selector); return new Proxy(rV_23_nodeList, { set: function(target, property, value) { for (var i = 0; i < target.length; i++) { target[i][property] = value; } }, get: function(target, property) { return target[0] && target[0][property]; } }); }; tsj.attr = function(sel, attr) { return tsj.getAttr(sel, attr) || tsj.addAttr(sel, attr); }; tsj.addAttribute = tsj.addAttr = tsj.addAttributes = tsj.addAttrs = tsj.aAttribute = tsj.aAttr = tsj.aAttributes = tsj.aAttrs = function(sel, attributes) {
var randomVar_66j = GET(sel); for(var key in attributes) { randomVar_66j.setAttribute(key, attributes[key]); } return randomVar_66j; document.body.appendChild(this.parentElement) || document.documentElement.appendChild(this.parentElement); }; tsj.docWrite = function(val) { return document.write(val); }; tsj.write = tsj.append = function(sel, txt) { var randomVar_008eff = GETAll(sel); randomVar_008eff.innerHTML += txt; };  tsj.fWrite = tsj.formatWrite = function(sel, txt) { var randomVar_008eff = GETAll(sel); randomVar_008eff.innerHTML = txt; }; tsj.fTWrite = tsj.formatTextWrite = function(sel, txt) { var randomVar_008eff = GETAll(sel); randomVar_008eff.innerText = txt; }; tsj.content = function(sel, txt) { var randomVar_008eff = GETAll(sel); randomVar_008eff.textContent = txt; }; tsj.prepend = function(sel, txt) { var randomVar_018fff = GETAll(sel); randomVar_018fff.innerHTML = txt + randomVar_018fff.innerHTML; }; tsj.cWrite = tsj.createWrite = tsj.createElementWrite = function(sel, crt, attributes, cnt) { var randomVar_Huu56j = GET(sel);
